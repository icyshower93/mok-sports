...
export default function MainPage() {
  ...
  const userTotalPoints = userStats?.totalPoints || 0;
  const userRank = userStats?.rank || 0;
  const userSkinsWon = userStats?.skinsWon || 0;
  const weeklyPrize = 30; // Static $30 per week as per Mok rules
  const teamsLeftToPlay = teamsLeftData?.teamsLeftToPlay || [];
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <Card className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-950/20 dark:to-indigo-950/20 border-blue-200 dark:border-blue-800">
              <CardContent className="p-4 text-center">
                <div className="text-2xl font-bold text-blue-700 dark:text-blue-300">{userTotalPoints}</div>
                <div className="text-xs text-blue-600 dark:text-blue-400 font-medium">Total Points</div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-950/20 dark:to-orange-950/20 border-amber-200 dark:border-amber-800">
              <CardContent className="p-4 text-center">
                <div className="flex items-center justify-center space-x-1">
                  <span className="text-2xl font-bold text-amber-700 dark:text-amber-300">#{userRank}</span>
                  {userRank === 1 && <Crown className="w-4 h-4 text-amber-600" />}
                </div>
                <div className="text-xs text-amber-600 dark:text-amber-400 font-medium">League Rank</div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-gradient-to-br from-emerald-50 to-green-50 dark:from-emerald-950/20 dark:to-green-950/20 border-emerald-200 dark:border-emerald-800">
              <CardContent className="p-4 text-center">
                <div className="text-2xl font-bold text-emerald-700 dark:text-emerald-300">{userSkinsWon}</div>
                <div className="text-xs text-emerald-600 dark:text-emerald-400 font-medium">Skins Won</div>
              </CardContent>
            </Card>
          </div>
        </div>

        {/* Weekly Rankings - Similar to League Page Design */}
        <Card>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-3 flex-1">
                        ...
                        <div className={`w-7 h-7 rounded-full flex items-center justify-center text-sm font-bold ${
                          index + 1 === 1 ? 'bg-yellow-500 text-white' :
                          index + 1 === 2 ? 'bg-gray-400 text-white' :
                          index + 1 === 3 ? 'bg-orange-500 text-white' :
                          'bg-muted text-muted-foreground'
                        }`}>
                          {index + 1 === 1 && <Crown className="w-3 h-3" />}
                          {index + 1 !== 1 && (index + 1)}
                        </div>
          ...
          <CardContent className="space-y-3 p-4">
            {teamsLeftToPlay.length === 0 ? (
              <div className="text-center py-6 text-muted-foreground">
                ...
                <p className="text-sm">All teams have played this week</p>
              </div>
            ) : (
              <div className="space-y-3">
                ...
                ).map(([ownerName, teams]: [string, any[]]) => (
